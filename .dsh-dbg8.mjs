...
const PORT = 9460;
const OUT = "C:\\Users\\94350\\deep-abyss\\.dsh-tmp\\dbg8";
...
const r = await ev(`(() => {
  document.getElementById('btn-help').click();     // 先关帮助，排除 helpOpen 干扰
  document.getElementById('btn-auto').click();
  return {
    oapCalls: window.__oap || 0,
    panelExists: window.__oapPanel,
    step1: window.__oapStep,
    step2: window.__oapStep2,
    hidden: document.getElementById('auto-panel').classList.contains('hidden'),
    guard: window.__guardVals,
  };
})()`);
console.log("插桩结果:");
