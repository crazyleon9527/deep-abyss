...
const PORT = 9455;
const OUT = "C:\\Users\\94350\\deep-abyss\\.dsh-tmp\\dbg7";
...
// 直接调用 onclick，让异常真抛出来
const r = await ev(`(() => {
  const errs = [];
  const old = window.onerror;
  window.onerror = (m, s, l, c, e) => { errs.push(String(m) + ' | ' + (e && e.stack ? e.stack.split('\\n').slice(0,4).join(' <- ') : '')); return false; };
  const btn = document.getElementById('btn-auto');
  let thrown = null;
  try { btn.onclick(); } catch (e) { thrown = String(e) + ' | ' + (e.stack||'').split('\\n').slice(0,5).join(' <- '); }
  window.onerror = old;
  return { thrown, errs, hidden: document.getElementById('auto-panel').classList.contains('hidden') };
})()`);
console.log("直接调用 onclick()（异常会真抛）:");
