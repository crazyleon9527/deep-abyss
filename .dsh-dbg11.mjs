...
const PORT = 9475;
const OUT = "C:\\Users\\94350\\deep-abyss\\.dsh-tmp\\dbg11";
...
const out = await ev(`(() => {
  ...
  const ctrlBtn = document.getElementById('btn-cast');
  const pauseBtn = document.getElementById('btn-pause');
  const sonarBtn = document.getElementById('btn-sonar');
  r.onclicks = {
    help: typeof helpBtn.onclick, auto: typeof autoBtn.onclick, cast: typeof ctrlBtn.onclick,
    pause: typeof pauseBtn.onclick, sonar: typeof sonarBtn.onclick,
  };
  r.autoDisabled = autoBtn.disabled;
  r.castDisabled = ctrlBtn.disabled;
  r.helpMaskVisible = !document.getElementById('help-mask').classList.contains('hidden');
  return r;
})()`);
console.log("绑定与禁用状态:", JSON.stringify(out, null, 1));
