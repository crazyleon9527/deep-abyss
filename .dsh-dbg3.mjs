...
const PORT = 9435;
const OUT = "C:\\Users\\94350\\deep-abyss\\.dsh-tmp\\dbg2";
...
const v = await ev(`(() => {
  const out = {};
  out.loadErrors = window.__errs.slice();
  out.autoPanel = !!document.getElementById('auto-panel');
  out.btnAuto = !!document.getElementById('btn-auto');
  out.btnAutoOnclick = document.getElementById('btn-auto') ? String(typeof document.getElementById('btn-auto').onclick) : 'no-btn';
  out.chipCount = document.querySelectorAll('#auto-panel [data-auto-n]').length;
  // 直接点一下，捕获异常
  try {
    document.getElementById('btn-auto').click();
    out.clickOk = true;
  } catch (e) { out.clickErr = String(e); out.clickOk = false; }
  const p = document.getElementById('auto-panel');
  out.afterClick = { hidden: p.classList.contains('hidden'), display: getComputedStyle(p).display, cls: p.className };
